...
import com.poloplan.service.EjercicioWaterpoloService;

import jakarta.validation.Valid;

@RestController
@RequestMapping("/api/ejercicios/waterpolo")
public class EjercicioWaterpoloController {

  private final CurrentUserService currentUserService;
  private final EjercicioWaterpoloService service;

  public EjercicioWaterpoloController(CurrentUserService currentUserService, EjercicioWaterpoloService service) {
    this.currentUserService = currentUserService;
    this.service = service;
  }

  @PostMapping
  public ResponseEntity<EjercicioWaterpoloResponse> crear(
    Authentication authentication,
    @Valid @RequestBody CrearEjercicioWaterpoloRequest request
  ) {
  ...
  @GetMapping
  public ResponseEntity<List<EjercicioWaterpoloResponse>> listar(Authentication authentication) {
    AppUser user = currentUserService.requireUser(authentication);
    return ResponseEntity.ok(service.listar(user));
  }

  @GetMapping("/{id}")
  public ResponseEntity<EjercicioWaterpoloResponse> obtener(Authentication authentication, @PathVariable Long id) {
    AppUser user = currentUserService.requireUser(authentication);
    return ResponseEntity.ok(service.obtener(user, id));
  }

  @PutMapping("/{id}")
  public ResponseEntity<EjercicioWaterpoloResponse> actualizar(
    Authentication authentication,
    @PathVariable Long id,
    @Valid @RequestBody ActualizarEjercicioWaterpoloRequest request
  ) {
    AppUser user = currentUserService.requireUser(authentication);
