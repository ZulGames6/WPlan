...
import com.poloplan.service.EjercicioGimnasioService;

import jakarta.validation.Valid;

@RestController
@RequestMapping("/api/ejercicios/gimnasio")
public class EjercicioGimnasioController {

  private final CurrentUserService currentUserService;
  private final EjercicioGimnasioService service;

  public EjercicioGimnasioController(CurrentUserService currentUserService, EjercicioGimnasioService service) {
    this.currentUserService = currentUserService;
    this.service = service;
  }

  @PostMapping
  public ResponseEntity<EjercicioGimnasioResponse> crear(
    Authentication authentication,
    @Valid @RequestBody CrearEjercicioGimnasioRequest request
  ) {
  ...
  @GetMapping
  public ResponseEntity<List<EjercicioGimnasioResponse>> listar(Authentication authentication) {
    AppUser user = currentUserService.requireUser(authentication);
    return ResponseEntity.ok(service.listar(user));
  }

  @GetMapping("/{id}")
  public ResponseEntity<EjercicioGimnasioResponse> obtener(Authentication authentication, @PathVariable Long id) {
    AppUser user = currentUserService.requireUser(authentication);
    return ResponseEntity.ok(service.obtener(user, id));
  }

  @PutMapping("/{id}")
  public ResponseEntity<EjercicioGimnasioResponse> actualizar(
    Authentication authentication,
    @PathVariable Long id,
    @Valid @RequestBody ActualizarEjercicioGimnasioRequest request
  ) {
    AppUser user = currentUserService.requireUser(authentication);
